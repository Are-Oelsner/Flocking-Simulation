...
public class Simulator {
        ...
        private static Collision checkIntersection(
                        Vector p, Vector r, Vector[] edge) {
                Collision c  = new Collision(Double.POSITIVE_INFINITY,null);
                r.subeq(p);
                edge[1].subeq(edge[0]);
                Vector qMinusP = edge[0].sub(p);
                double rCrossS = r.cross(edge[1]);
                double t = qMinusP.cross(edge[1])/rCrossS;
                double u = qMinusP.cross(r)/rCrossS;
                if(equals(rCrossS, 0)) {
                        System.out.println("No Collision detected");
                        return c;
        ...
        public static void main(String[] args) {
                ...
                //Test of intersection detection with Vector[] edge input
                //TODO
        }
}
